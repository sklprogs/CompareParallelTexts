...
def dialog_open_file(dir,my_title=None,mask='*',PersistPar=False,Critical=False):
	cur_func = sys._getframe().f_code.co_name
	file = ''
	if globs['AbortAll']:
...
# Make a tag visible
def drag_screen(widget,mark_name='mark',pane_no=1):
	cur_func = sys._getframe().f_code.co_name
	if globs['AbortAll']:
		log(cur_func,lev_warn,globs['mes'].abort_func % cur_func)
	else:
		try:
			widget.yview(mark_name)
		except:
			log(cur_func,lev_err,globs['mes'].shift_screen_failure2 % (pane_no,mark_name))
				...
				if text_db['len_search_list'] > 0:
					...
					if len(text_db['search_list']) > 0:
						...
						if globs['bool']['UseHeadInterval']:
							...
							if lift_pos - globs['int']['max_head_interval'] >= 0:
								lift_pos -= globs['int']['max_head_interval']
						lift_pos = pos2tk(text_db,lift_pos,Even=False)
						mark_add(widget=widget,mark_name='lift_pos',postk=lift_pos)
...
def text_field(title=None,user_text=err_mes_empty_input,CheckSpelling=False,GoTo='',Insist=False,SelectAll=False,ReadOnly=False,Small=False,TrimEnd=False,CursorTk='1.0',xoffset=0,yoffset=0): # Edit=True равноценно user_text!=err_mes_empty_input
	...
	if globs['AbortAll']:
		...
		top.title(title)
		top.tk.call('wm','iconphoto',top._w,tk.PhotoImage(file=globs['var']['icon_cpt']))
...
def default_config(config='cpt',Init=True):
	cur_func = sys._getframe().f_code.co_name
	if Init:
		globs['lin'] = {}
		globs['win'] = {}
	...
	globs['cpt_config'] = globs['bin_dir'] + sysdiv + globs['cpt_config_root']
	if config == 'cpt':
		globs['lin'].update({'home':'/home/pete'})
		#----------------------------------------------------------------------
		globs['win'].update({'home':r'C:\Documents and Settings\user'})
		#----------------------------------------------------------------------
		globs['mac'].update({'home':'/home/pete'})
		#----------------------------------------------------------------------
		globs['bool'].update({'AlwaysMaximize':True,
		                      'DeleteBOM':True,
		                      'ReadOnlyProtection':False,
		                      'Spelling':False,
		                      'TextButtons':True,
		                      'UnixSelection':False,
		                      'UseHeadInterval':True
		                    })
		#----------------------------------------------------------------------
		globs['int'].update({ 'default_button_size':36,
                              'default_hint_border_width':1,
                              'default_hint_delay':800,
                              'default_hint_height':40,
                              'default_hint_width':280,
                              'max_head_interval':1000
		                   })
		#----------------------------------------------------------------------
		globs['var'].update({ 'bind_re_search_article':'<Control-F3>',
...
def read_configs(Silent=False,Critical=False):
	...
	if globs['AbortAll']:
		...
		cur_mes=''
		if not exist(globs['cpt_config'],Silent=Silent,Critical=Critical):
			Success=False
		if Success:
			try:
				globs['config_parser'].readfp(codecs.open(globs['cpt_config'],'r','utf-8'))
			except:
				Success=False
				mestype(cur_func,globs['mes'].invalid_config % globs['cpt_config'],Silent=Silent,Critical=Critical)
			if Success:
				if sys_type=='lin':
...
def compare_gui(text1_db,text2_db,BothPanes=True):
	def compare_callback1():
		...
		if globs['AbortAll']:
			...
			if stone_no2  != - 1:
				...
				mark_add(widget=txt2,mark_name='insert',postk=fragm_start)
				# Remember the last successful/found selection
				cursor_pos = get_cursor(widget=txt2,pane_no=2)
				cursor_pos = tk2pos(text2_db,cursor_pos,Even=False)
				get_word_by_pos(text2_db,cursor_pos)
				mark_add(widget=txt2,mark_name='lift_pos',postk=lift_pos)
				drag_screen(widget=txt2,mark_name='lift_pos',pane_no=2)
	#--------------------------------------------------------------------------
	def compare_callback2():
		...
		if globs['AbortAll']:
			...
			if stone_no2  != - 1:
				...
				fragm_end = pos2tk(text1_db,fragm_end,Even=True)
				tag_add_config(widget=txt1,tag_name='txt1tag',pos1tk=fragm_start,pos2tk=fragm_end,mode='bg',color='orange')
				mark_add(widget=txt1,mark_name='insert',postk=fragm_start)
				# Remember the last successful/found selection
				cursor_pos = get_cursor(txt1,pane_no=1)
				cursor_pos = tk2pos(text1_db,cursor_pos,Even=False)
				get_word_by_pos(text1_db,cursor_pos)
				mark_add(widget=txt1,mark_name='lift_pos',postk=lift_pos)
				drag_screen(widget=txt1,mark_name='lift_pos',pane_no=1)
	#--------------------------------------------------------------------------
	# Find a match in a parallel text directly after a pane search
	def instant_search(text_db,widget,direction='forward',pane_no=1): # clear, forward, backward
		cur_func = sys._getframe().f_code.co_name
		if globs['AbortAll']:
			log(cur_func,lev_warn,globs['mes'].abort_func % cur_func)
		else:
			search_pane(text_db=text_db,widget=widget,direction=direction,pane_no=pane_no)
			if pane_no == 1:
				compare_callback1()
			else:
				compare_callback2()
	...
	if globs['AbortAll']:
		...
		#create_button(parent_widget=txt1,text=globs['mes'].btn_search,hint=globs['mes'].hint_search_article,action=lambda e:instant_search(text1_db,txt1,direction='clear'),icon_path=globs['var']['icon_search_article'],bindings=globs['var']['bind_re_search_article'])
		create_binding(widget=txt1,bindings=globs['var']['bind_search_article_forward'],action=lambda e:instant_search(text1_db,txt1,direction='forward',pane_no=1))
		create_binding(widget=txt1,bindings=globs['var']['bind_search_article_backward'],action=lambda e:instant_search(text1_db,txt1,direction='backward',pane_no=1))
		create_binding(widget=txt1,bindings=globs['var']['bind_re_search_article'],action=lambda e:instant_search(text1_db,txt1,direction='clear',pane_no=1))
...
# todo: Determination of numeration directly in words (for example, chemical compounds). In this case the direct order of digits within the word is required
def compare_parallel_texts(orig_file,transl_file,MakePretty=True,BothPanes=True,Silent=False,Critical=True):
	cur_func = sys._getframe().f_code.co_name
	if globs['AbortAll']:
		log(cur_func,lev_warn,globs['mes'].abort_func % cur_func)
	else:
		if exist(orig_file,Silent=Silent,Critical=Critical) and exist(transl_file,Silent=Silent,Critical=Critical):
			...
			# ------------------------------------------------------------------
			root.withdraw()
			compare_gui(text1_db,text2_db,BothPanes=BothPanes)
			#root.deiconify()
			# tmp # Standalone only!
			sys.exit()
...
globs = {'cpt_config_root':'CompareParallelTexts.cfg','config_parser':SafeConfigParser(),'AbortAll':False,'mes':mes_ru,'ui_lang':'ru'}
my_program_title = ''

root = tk.Tk()
root.title(globs['mes'].wait)

default_config(config='cpt',Init=True)
read_configs()
